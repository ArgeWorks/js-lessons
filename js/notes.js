...
// OBJECT ASSIGN
// Объединяет 2 обекта, при этом если во втором объекте есть такие же поля как и в первом объекте, то поля в первом объекте заменяются на поля из второго объекта
let obj = Object.assign({ name: 'Unknown', age: '28' }, { name: 'Igor' }); // Получим { name: 'Igor', age: '28' }


// ПЕРЕМЕННЫЕ В ВИДЕ ОБЪЕКТА
// Что-бы присвоить значение нужно передать объект с такими же именами полей как в переменной, эти переменные можно использовать как обычные переменные (вне объекта)
let { top, bottom } = { top: 5, bottom: 7 };
console.log(top); // 5
console.log(bottom); // 7
...
//////////////////////////////////////////////////////////// jQuery ////////////////////////////////////////////////////////////
// У большинства методов в jQuery возвращаются jQuery объекты, к ним можно продолжить назначать другие jQuery действия по цепочке.

// let divs = document.querySelectorAll('div');
let divs = $('div')

// divs.forEach(div => div.addEventListener('click', handler));
divs.on('click', handler); // При клике на разные элементы this будет равен тому элементу на который произошел клик (аналогично e.target)
// или
divs.click(handler); // Так можно использовать практически любое событие

// div.removeEventListener('click', handler)
divs.off('click'); // Имя обработчика и селектор передавать не обязательно

// Делегирование
// div.addEventListener('click', (e) => { if (e.target.classList.contains('innerElement')) console.log('Это элемент внутри дива') })
div.on('click', '.innerElement', handler);

// При клике на один элемент событие клик произовйдет и на всех остальных элементах
divs.trigger('click'); 

// Получаем элемент с определенным атрибутом
$('div').attr('class') // Ищет элемент у которого есть аттрибут class
$('div').attr('class', 'btn') // Записывает в атрибут class значение btn (если там были другие значения то они перезапишуться)
...
// Сворачивание\разворачивание блока при клике на кнопку
btn.on('click', () => {
    block.slideToggle(1000); // Первым аргументом передается время за которое отрабатывает анимация, вторым параметром можно передать коллбек который отработает при завершении анимации
})

// Объединение двух объектов. Аналог Object.assign
let obj = $.extend({ name: 'Unknown', age: '28' }, { name: 'Igor' }); // Получим { name: 'Igor', age: '28' }
